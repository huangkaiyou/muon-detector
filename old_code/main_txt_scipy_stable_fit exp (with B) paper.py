...
analysis_result_dir = './analysis_result/lifetime/with B/scipy/' + f'threshold_second ({threshold_second})' + mode + prominice_value + ground_value + '/analysis'
analysis_figure_dir = './analysis_result/lifetime/with B/scipy/' + f'threshold_second ({threshold_second})' + mode + prominice_value + ground_value + '/frame'
...
for file_name in file_names:    
    ...
    try:
        ...
        # for i in range(10000):
            ...
            #second peak finding
            ch1_second_peak_index, ch1_second_peak_info = find_peaks(raw3, height= threshold_second, prominence= np.std(raw3) * 2)
            ch2_second_peak_index, ch2_second_peak_info = find_peaks(raw4, height= threshold_second, prominence= np.std(raw4) * 2)
